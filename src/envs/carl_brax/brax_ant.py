'''
Copy of Brax Ant found in folder brax/envs/ant.py.
Modified to be compatible with jax version 0.4.27.
'''
...
class Ant(PipelineEnv):
  ...
  def reset(self, rng: jp.ndarray) -> State:
    ...
    print(f'(brax_ant.reset) rng = {rng}, rng.shape = {rng.shape}, rng.size = {rng.size}')
    print(f'(brax_ant.reset) isinstance(rng,jp.ndarray) = {isinstance(rng, jp.ndarray)}')
    if rng.size == 1:
      # print(f'(brax_ant.reset) rng.size = {rng.size}, rng[0] = {rng[0]}')
      rng = jax.random.PRNGKey(0)
      print(f'(brax_ant.reset) Overridding rng. New rng = {rng}')
      print(f'(brax_ant.reset) rng = {rng}, rng.shape = {rng.shape}, rng.size = {rng.size}')
    # if rng.size == 0:
    #   rng = jax.random.PRNGKey(0)
    #   print(f'(brax_ant.reset) Overridding rng. New rng = {rng}')
