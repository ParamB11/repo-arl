...
from brax.envs.wrappers import training #custom
...
from envs.carl_brax.brax_ant import Ant #custom
        ...
        **kwargs,
        ...
        if env is None:
            ...
            if self.env_name == 'ant':
                print(f'(brax_base_env.init) batch_size, bs = {bs}')
                env = Ant(backend=self.backend)
                env = training.EpisodeWrapper(env, episode_length=1000, action_repeat=1)
                env = training.VmapWrapper(env, batch_size=bs)
...
# from brax_ant import Ant #custom
