    ...
    def get_context_features() -> dict[str, ContextFeature]:
        return {
            ...
            "mass_torso": UniformFloatContextFeature(
                "mass_torso", lower=1e-6, upper=np.inf, default_value=6.25 #10
            ),
            "mass_bthigh": UniformFloatContextFeature(
                "mass_bthigh", lower=1e-6, upper=np.inf, default_value=1.54 #1.5435146
            ),
            "mass_bshin": UniformFloatContextFeature(
                "mass_bshin", lower=1e-6, upper=np.inf, default_value=1.58 #1.5874476
            ),
            ...
            "mass_fthigh": UniformFloatContextFeature(
                "mass_fthigh", lower=1e-6, upper=np.inf, default_value=1.43 #1.4380753
            ),
            "mass_fshin": UniformFloatContextFeature(
                "mass_fshin", lower=1e-6, upper=np.inf, default_value=1.20 #1.2008368
            ),
            "mass_ffoot": UniformFloatContextFeature(
                "mass_ffoot", lower=1e-6, upper=np.inf, default_value=0.88 #0.8845188
            ),
        }