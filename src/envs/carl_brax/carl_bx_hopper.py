...
# print(f'carl_bx_hopper.py: cwd = {os.getcwd()}')

class CARLBraxHopper(CARLBraxEnv):
    env_name: str = "hopper"
    asset_path: str = "envs/assets/hopper.xml"
    # asset_path: str = os.path.join(os.getcwd(), "crl_py/envs/carl_brax/hopper.xml")
    ...
    def get_context_features() -> dict[str, ContextFeature]:
        ...
        return {
            ...
            "friction": UniformFloatContextFeature(
                "friction", lower=0, upper=100, default_value=0.6 #1
            ),
            ...
            "mass_torso": UniformFloatContextFeature(
                "mass_torso", lower=1e-6, upper=np.inf, default_value=3.6651914 #3.53429174 #10
            ),
            "mass_thigh": UniformFloatContextFeature(
                "mass_thigh", lower=1e-6, upper=np.inf, default_value=4.0578904 #3.92699082 #4.0578904
            ),
            "mass_leg": UniformFloatContextFeature(
                "mass_leg", lower=1e-6, upper=np.inf, default_value=2.7813568 #2.71433605 #2.7813568
            ),
            "mass_foot": UniformFloatContextFeature(
                "mass_foot", lower=1e-6, upper=np.inf, default_value=5.3155746 #5.0893801 #5.3155746
            ),
        }