...
class LSTMContextPredictor(nn.Module):
    # def __init__(self, input_size, lstm_hidden_size, hidden_layers, output_size, dropout_prob=0.0, device='cpu'):
    def __init__(self, input_size, hidden_size, out_size, device='cpu'):
        ...
        # self.layers = nn.ModuleList([nn.LSTM(input_size, lstm_hidden_size).to(device)])
        # self.dropout_layers = nn.ModuleList([nn.Dropout(p=dropout_prob)])
        # self.layers.append(nn.Linear(lstm_hidden_size, hidden_layers[0]).to(device))
    ...
    def forward(self, x, h0=None, c0=None):
        ...
        # print(f'hn.size={hn.size()}, cn.size={cn.size()}')
        # print(f'hn={h0},\ncn={cn}')
        return output, (hn, cn)
...
class RecurrentContextPredictor(nn.Module):
    ...
    def forward(self, x, h0=None, c0=None):
        # if h0 is None or c0 is None:
        #     h0, c0 = torch.zeros(1, x.shape[0], self.recurrent_hidden_size), torch.zeros(1, x.shape[0], self.recurrent_hidden_size)
        #     h0, c0 = h0.to(self.device), c0.to(self.device)
        if h0 is None:
        ...
        if self.model_arch == 'lstm':
            ...
            # print(f'x.shape={x.shape},{x.size()}, h0.shape={h0.shape}, {h0.size()}')
